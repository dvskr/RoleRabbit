    ...
    <div className={`${sidebarCollapsed ? 'w-16' : 'w-72'} bg-gradient-to-b from-blue-50/30 to-purple-50/30 border-r border-blue-200/30 shadow-xl flex flex-col`}>
      ...
      <nav className={`flex-1 ${sidebarCollapsed ? 'p-2' : 'p-3'} space-y-1 overflow-y-auto`}>
        ...
        <button 
          ...
          className={`w-full flex items-center ${sidebarCollapsed ? 'justify-center px-2 py-3' : 'gap-3 px-4 py-3.5'} rounded-xl ${activeTab === 'learning' ? 'bg-gradient-to-r from-blue-500 to-purple-500 text-white shadow-lg shadow-blue-500/30 ' : 'text-gray-700 hover:bg-gradient-to-r hover:from-pink-500 hover:to-purple-500 hover:text-white hover:shadow-lg hover:shadow-pink-500/30'}`}
        ...
        >
          <Bot size={20} />
          {!sidebarCollapsed && <span className="font-semibold">AI Agents</span>}
        </button>
      </nav>
    </div>
  );
